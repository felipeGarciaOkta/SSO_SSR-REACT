  ...
  useEffect(() => {
    ...
    if (!authState) {
      ...
      if (authState.isAuthenticated) {
        ...
        if (redirect) {
          console.log("This is where redirection happens");
        ...
        if (flow == "initialCheck") {
          //http://localhost:8080/login?redirect=http://localhost:3030/login&flow=initalCheck
          if (origReferer) {
            ...
            const redirect = localStorage.getItem('redirect');
            //http://localhost:9080/login?redirect=http://localhost:8080/login&flow=initalCheck&&origReferer=http://localhost:3030/login
            if (redirect) {
              const newRedirect = "http://localhost:9080/login";

              const url = new URL(newRedirect);
              url.searchParams.append('redirect', 'http://localhost:8080/requestToken');
    ...
    if (!username || !password) {
      ...
      try {
        ...
        if (!sessionToken) {
          ...
          //
          /*
          const url = new URL("http://localhost:5050/login2");
