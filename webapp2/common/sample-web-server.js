...
module.exports = function SampleWebServer(sampleConfig, extraOidcOptions, homePageTemplateName) {
  ...
  app.get('/', (req, res) => {
    ...
    const checkedSession = req.query.checkedSession === 'true';
  ...
  app.get('/tokens', oidc.ensureAuthenticated(), (req, res) => {
    // Convert the userinfo object into an attribute array, for rendering with mustache
    const userTokens = req.userContext.tokens;
    const attributes = Object.entries(userTokens);
    const expiresAt = new Date(userTokens.expires_at * 1000).toLocaleString();

    console.log("Expo: "+expiresAt);
    ...
    oktaJwtVerifier.verifyAccessToken(userTokens.access_token, 'api://default')
      .then(jwt => {
        // the token is valid (per definition of 'valid' above)
        console.log("Access Tokens");
        accesstoken = Object.entries(jwt.claims);
    ...
    oktaJwtVerifier.verifyIdToken(userTokens.id_token, sampleConfig.oidc.clientId)
      .then(jwt => {
        // the token is valid (per definition of 'valid' above)
        console.log("Id Tokens");
        idtoken = Object.entries(jwt.claims);
        console.log(idtoken);
      })
      .catch(err => {
